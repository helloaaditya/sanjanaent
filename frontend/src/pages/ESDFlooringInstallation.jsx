...
import { Zap, CheckCircle, Award, Phone, ArrowRight, Wrench, Eye, Target, Sparkles, Hammer, Shield, Lock, MapPin } from 'lucide-react'
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6">
            <ScrollAnimation animation="fade-in-up" delay={0}>
              <div className="flex items-center gap-2 text-yellow-600 mb-6">
                <MapPin size={20} />
                <h2 className="text-2xl font-black text-gray-900">ESD Flooring Installation Across Bangalore</h2>
              </div>
              <p className="text-gray-600 mb-6">We serve all major areas. Select your locality or find service near you:</p>
              <div className="mb-4">
                <Link to="/esd-flooring-installation-near-me" className="inline-flex items-center gap-2 text-yellow-600 font-semibold hover:text-yellow-700">
                  <MapPin size={18} />
                  ESD Flooring Near Me
                </Link>
              </div>
              <div className="flex flex-wrap gap-3">
                {BANGALORE_AREAS.map((area) => (
                  <Link key={area.slug} to={`/esd-flooring-installation-${area.slug}-bangalore`} className="px-4 py-2 bg-yellow-50 text-yellow-700 rounded-lg text-sm font-medium hover:bg-yellow-100 transition-colors">
                    {area.name}
                  </Link>
                ))}
              </div>
