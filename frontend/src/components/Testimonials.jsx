      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
          {loading ? (
            ...
            items.map((t) => (
              <div key={t._id} className="bg-slate-50 border border-slate-200 rounded-xl p-5">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
          ...
          {error && (
            <div className="flex items-center text-red-600 mb-3"><AlertCircle className="mr-2" size={18} /> {error}</div>
          )}
          {success && (
            <div className="flex items-center text-green-600 mb-3"><CheckCircle2 className="mr-2" size={18} /> {success}</div>
          )}
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="inline-flex items-center bg-blue-600 hover:bg-blue-700 text-white font-semibold px-5 py-2.5 rounded-lg disabled:opacity-60"
            >
              {submitting ? (<><Loader2 className="animate-spin mr-2" size={18} /> Submitting...</>) : (<><Send className="mr-2" size={18} /> Submit for Approval</>)}
            </button>
          </form>
        </div>
      </div>
